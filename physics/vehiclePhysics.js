...
export class VehiclePhysics {
  ...
  updateTireForces(dt) {
    ...
    for (const wheel of this.wheels) {
      ...
      const availableLongitudinal =
        effectiveLoad * this.config.tireGrip * this.config.longitudinalGrip * surfaceGrip;
      const availableLateral =
        effectiveLoad * this.config.tireGrip * this.config.lateralGrip * surfaceGrip;
  ...
  applyEngineForce(dt) {
    ...
    const ramp = targetDriveInput === 0 ? 8.6 : targetDriveInput < 0 ? 4.2 : 6.2;
